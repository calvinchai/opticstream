...
from opticstream.artifacts.oct import publish_project_artifact_task
...
from opticstream.flows.psoct.utils import mosaic_ident_from_project_and_mosaic_id
...
def check_batch_state_task(
    ...
    processed_batches = sum(1 for b in batches if b.enface_processed)
...
@task(task_run_name="{project_name}-slice-{slice_id}-check-state")
def check_slice_state_task(
    ...
    normal_mosaic_id = 2 * slice_id - 1
    tilted_mosaic_id = 2 * slice_id
    ...
    logger.info(
        f"Slice {slice_id}: normal_complete={normal_complete}, "
...
def check_completion_and_emit_mosaic_ready_task(
    ...
    emit_event(
        ...
        payload={
            ...
            "project_base_path": project_base_path,
            "mosaic_id": mosaic_id,
            "mosaic_ident": mosaic_ident_from_project_and_mosaic_id(
                project_name, mosaic_id
...
def manage_mosaic_batch_state_flow(
    ...
    artifact_key = publish_project_artifact_task(
        project_ident=OCTProjectId(project_name=project_name),
    )
...
@flow(flow_run_name="{project_name}-slice-{slice_id}-manage-state")
def manage_slice_state_flow(
    ...
    """
    logger = get_run_logger()
    logger.info(f"Managing state for slice {slice_id}")
    ...
    # Update unified project mosaic artifact (includes stitching status)
    unified_artifact_key = publish_project_artifact_task(
        project_ident=OCTProjectId(project_name=project_name),
    )
    ...
    if both_stitched:
        logger.info(
            f"Both mosaics stitched for slice {slice_id}. "
            f"Emitting {SLICE_READY} event."
        )
        emit_event(
            event=SLICE_READY,
            resource={
                "prefect.resource.id": f"slice:{project_name}:slice-{slice_id}",
                "project_name": project_name,
...
def manage_slice_state_event_flow(
    ...
    """
    # Determine slice id from mosaic_id
...
def unified_state_management_event_flow(
    ...
    payload : dict
        ...
        - slice_id: int (optional, for slice events)
    ...
    elif event == MOSAIC_ENFACE_STITCHED:
        logger.info(
