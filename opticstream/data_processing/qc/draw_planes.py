...
def load_volume(
    ...
    if suffix not in [".nii", ".nii.gz", ".nifti"]:
        raise ValueError(
            f"Unsupported file format: {suffix}. Only NIfTI files (.nii, .nii.gz) are supported."
        )
    ...
    if data.ndim != 3:
        raise ValueError(
            f"Expected 3D volume, but got {data.ndim}D array with shape {data.shape}"
        )
...
def draw_planes(
    volume: np.ndarray, zmap: np.ndarray, thickness: int = 1, value: float = 0.0
) -> np.ndarray:
    """
...
def save_volume(
    ...
    if suffix not in [".nii", ".nii.gz", ".nifti"]:
        raise ValueError(
            f"Unsupported output format: {suffix}. Only NIfTI files (.nii, .nii.gz) are supported."
        )
...
def main(
    ...
    print(
        f"  Shape: {vol_data.shape}, dtype: {vol_data.dtype}, range: [{vol_data.min():.2f}, {vol_data.max():.2f}]"
    )

    print(f"Loading z-index map from {zmap}...")
    zmap_data = load_zmap(zmap)
    print(
        f"  Shape: {zmap_data.shape}, dtype: {zmap_data.dtype}, range: [{zmap_data.min():.2f}, {zmap_data.max():.2f}]"
    )
    ...
    num_planes = (
        np.sum(~np.isnan(zmap_data))
        if zmap_data.dtype == np.float64
        else zmap_data.size
    )
    estimated_voxels = num_planes * thickness
    print(
        f"  Estimated {estimated_voxels} voxels modified across {num_planes} plane locations"
    )
