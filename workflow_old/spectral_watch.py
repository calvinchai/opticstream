...
def process_single_file_flow(
    ...
    """
    Process a single file: load with load_spectral_file_raw, then save with .gz and process in parallel.
    ...
    data = load_spectral_data_raw_task(file_content, aline_length, bline_length, nifti_header_size)
...
def folder_watch_flow(
    ...
    if process_existing:
        ...
        if existing_files:
            flow_logger.info(f"Found {len(existing_files)} existing files, emitting events")
...
if __name__ == "__main__":
    ...
    if args.command == "watch":
        ...
        print("1. Create an automation in Prefect UI that triggers 'spectral-process' deployment")
