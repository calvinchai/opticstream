...
from workflow.state import MosaicState, ProjectState, SliceState
...
def refresh_and_save_mosaic_state_task(
    ...
    if mosaic_id == slice_state.normal_mosaic_id:
        ...
        logger.warning(
            f"Mosaic {mosaic_id} doesn't match expected pattern for slice {slice_number}"
        )
        return False
    ...
    if success:
        logger.info(
            f"Successfully saved mosaic {mosaic_id} state to project state variable"
        )
    else:
        logger.warning(
            f"Failed to save mosaic {mosaic_id} state to project state variable"
        )
...
def refresh_and_save_slice_state_task(
    ...
    if success:
        logger.info(
            f"Successfully saved slice {slice_number} state to project state variable"
        )
    else:
        logger.warning(
            f"Failed to save slice {slice_number} state to project state variable"
        )
