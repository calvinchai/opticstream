...
FOLDER_TO_WATCH = "/mnt/sas2/I80_frontal_20251210/"  # TODO: set the correct folder
# path here
    ...
    for mosaic_id in sorted(files_by_mosaic.keys()):
        ...
        for i in range(0, len(file_list), batch_size):
            ...
            try:
                ...
                print(
                    f"Triggered deployment for mosaic {mosaic_id} with {len(batch_files)} files (batch {batch_id})")
            except Exception as e:
                print(
                    f"Failed to trigger deployment for mosaic {mosaic_id}, batch {batch_id}: {e}")
            break
